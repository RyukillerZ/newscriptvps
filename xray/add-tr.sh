...
if [ "$(grep -w "trojanws $user" "/root/.premium/.database" | wc -l)" != '0' ]; then
	sed -i "/trojanws $user/d" "/root/.premium/.database"
fi
echo "trojanws $user $exp $harini $user $get_AdminID $get_AdminID active" >>"/root/.premium/.database"
#############
